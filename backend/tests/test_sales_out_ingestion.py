...
def _sales_out_schema_available() -> bool:
    """True if migration 014 is applied (sales_out enum + sales_out_stage table)."""
    from sqlalchemy import text
    try:
        db = SessionLocal()
        try:
            r = db.execute(text(
                "SELECT 1 FROM pg_enum e JOIN pg_type t ON e.enumtypid = t.oid WHERE t.typname = 'ingestion_entity_enum' AND e.enumlabel = 'sales_out' LIMIT 1"
            ))
            if r.scalar() != 1:
                return False
            r2 = db.execute(text(
                "SELECT 1 FROM information_schema.tables WHERE table_name = 'sales_out_stage' LIMIT 1"
            ))
            return r2.scalar() == 1
        finally:
            db.close()
