...
| **plan_run_demand_inputs_weekly** | id, plan_run_id (FK), week_start, sku, warehouse_code, demand_qty, source, source_ref, demand_breakdown_json, demand_includes_samples, is_frozen | Materialized demand used by run; unique (plan_run_id, week_start, sku, warehouse_code). demand_breakdown_json: per-type (CUSTOMER/SAMPLES/ADJUSTMENT) + included/excluded, or OVERRIDE/FORECAST_TOTAL; demand_includes_samples from policy at resolve time. |
