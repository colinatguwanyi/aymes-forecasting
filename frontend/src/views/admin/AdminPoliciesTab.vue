  ...
  <div class="space-y-4">
    <PageHeader title="Stock Rules (SKU × Warehouse)" :breadcrumbs="[{ label: 'Settings', path: '/admin/policies' }]">
      <template #actions>
        ...
        <button type="button" class="btn-primary px-4" @click="openDrawer('add')">Add policy</button>
    ...
    <DrawerForm v-model="drawerOpen" :title="drawerMode === 'add' ? 'Add policy' : 'Edit policy'">
      ...
      <template #footer>
        ...
        <button type="button" class="btn-primary px-4" @click="submitDrawer">{{ drawerMode === 'add' ? 'Add policy' : 'Save' }}</button>
      </template>
    </DrawerForm>
  </div>
</template>
